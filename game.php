...
<head>
    ...
    <title>Rejoindre une Partie - La Chasse aux Sorcières</title>
</head>
...
<body>
    ...
    <script>
        function joinGame() {
            const gameStatus = document.getElementById('game-status').innerText;
            if (gameStatus === 'En attente de joueurs') {
                alert('Vous avez rejoint la partie!');
                document.getElementById('game-status').innerText = 'Partie en cours';
                document.getElementById('join-game-button').disabled = true;
            } else {
                alert('La partie a déjà commencé, vous pouvez rejoindre en spectateur.');
            }
        }
    </script>
</body>
